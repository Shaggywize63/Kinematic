...
export const getSubmission = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const { data, error } = await supabaseAdmin
    .from('form_submissions')
    .select('*, form_responses(*, builder_questions:builder_questions!field_id(title, qtype)), builder_forms:builder_forms!template_id(title), activities(name)')
    .eq('id', id)
    .single();
...
export const getAllSubmissions = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  let query = supabaseAdmin
    ...
    .select(`
      ...
      activities(name),
      form_responses(*, builder_questions:builder_questions!field_id(title))
    `, { count: 'exact' })
    .eq('org_id', user.org_id)
    .order('submitted_at', { ascending: false })
    .range(from, to);
