...
export const getMySubmissions = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  let query = supabaseAdmin
    .from('form_submissions')
    .select('*, form_templates:builder_forms!template_id(title), activities(name)', { count: 'exact' })
    .eq('user_id', user.id)
...
export const getSubmission = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const { data, error } = await supabaseAdmin
    .from('form_submissions')
    .select('*, form_responses(id, value_text, value_number, value_bool, photo_url, builder_questions:builder_questions!field_id(title, qtype)), builder_forms:builder_forms!template_id(title), activities(name)')
    .eq('id', id)
    .single();
...
export const getAllSubmissions = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  let query = supabaseAdmin
    ...
    .select(`
      ...
      builder_forms:builder_forms!template_id(title),
      activities(name),
      form_responses(id, value_text, value_number, value_bool, photo_url, builder_questions:builder_questions!field_id(title))
    `, { count: 'exact' })
    .eq('org_id', user.org_id)
    .order('submitted_at', { ascending: false })
    .range(from, to);
