...
/**
 * Tenant + actor context for {@link registerConsumerPurchase}. Supplied
 * explicitly (rather than read off `req.user`) so BOTH the authenticated admin
 * handler and the unauthenticated public capture route (`/distribution/capture`)
 * can share one implementation. `actor_id` is null for public self-registration.
 */
...
/**
 * The shared write path: serial resolution → registration row → best-effort
 * tertiary-sale + CRM-lead fan-out → back-pointer stitch. Throws on the primary
 * registration insert (the caller maps it to a 4xx); the two side-effects stay
 * best-effort so a CRM hiccup never fails the consumer-facing capture. Does NOT
 * write an audit row — the authed handler does that with its request context.
 */
export async function registerConsumerPurchase(
  input: RegistrationInput,
  ctx: RegistrationContext,
): Promise<{ reg: any; tertiary_sale_id: string | null; lead_id: string | null }> {
  // ── 1. Resolve serial (if any) → sku_id + retailer_id where possible.
  let serial_id   = input.serial_id ?? null;
  let sku_id      = input.sku_id ?? null;
  let retailer_id = input.retailer_id ?? null;

  if (!serial_id && input.serial_text) {
    const { data: matched } = await supabaseAdmin
      .from('distribution_sku_serials')
      .select('id, sku_id, current_holder_type, current_holder_id')
      .eq('org_id', ctx.org_id)
      .eq('serial', input.serial_text.trim())
      .maybeSingle();
    if (matched) {
  ...
  const { data: reg, error: regErr } = await supabaseAdmin
    ...
    .insert({
      serial_id,
      serial_text: input.serial_text ?? null,
      sku_id,
      retailer_id,
      consumer_phone: input.consumer_phone.trim(),
  ...
  // ── 4. Side-effect: CRM Lead so the consumer enters the lead pipeline.
  ...
  return { reg, tertiary_sale_id, lead_id };
}
...
export const create = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  let result: { reg: any; tertiary_sale_id: string | null; lead_id: string | null };
  try {
    result = await registerConsumerPurchase(parsed.data, {
      org_id: user.org_id,
      client_id: user.client_id ?? null,
      actor_id: user.id,
    });
  } catch (e) {
    return badRequest(res, e instanceof Error ? e.message : 'Registration failed');
  }

  await audit(req, 'consumer_registration.create',
              'distribution_consumer_registrations', result.reg.id, null,
              { ...result.reg, tertiary_sale_id: result.tertiary_sale_id, lead_id: result.lead_id });

  created(res, { ...result.reg, tertiary_sale_id: result.tertiary_sale_id, lead_id: result.lead_id });
});
