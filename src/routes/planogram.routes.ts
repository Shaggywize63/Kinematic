 ...
 * All routes require auth; analytics + management are scoped by org_id.
 */
...
const upsertPlanogramSchema = z.object({
  ...
  layout: z
    .object({ shelves: z.array(z.object({ index: z.number(), capacity: z.number().optional() })) })
    .optional(),
  expected_skus: z.array(
    z.object({
      sku_id: z.string(),
      sku_name: z.string(),
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const out = await PlanogramService.processCapture({
      ...
      capture: {
        lat: body.capture_lat,
        lng: body.capture_lng,
        deviceMeta: body.device_meta,
      },
    });
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    res.json({
      success: true,
      data: { capture: cap, recognition: rec, compliance: comp },
    });
  }),
);
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const data = await PlanogramAnalyticsService.orgTrend(req.user.org_id, days);
    res.json({ success: true, data });
  }),
);
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const data = await PlanogramAnalyticsService.storeRanking(req.user.org_id, days);
    res.json({ success: true, data });
  }),
);

router.get(
  '/analytics/chronic-gaps',
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const data = await PlanogramAnalyticsService.chronicGaps(req.user.org_id);
    res.json({ success: true, data });
  }),
);

router.get(
  '/analytics/sku-visibility',
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const days = Math.min(60, Math.max(1, Number(req.query.days) || 14));
    const data = await PlanogramAnalyticsService.skuVisibility(req.user.org_id, days);
    res.json({ success: true, data });
  }),
);
...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const data = await PlanogramAnalyticsService.riskForecast(req.user.org_id);
    res.json({ success: true, data });
  }),
);
