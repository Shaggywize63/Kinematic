...
router.get('/forms', requireAuth, asyncHandler(async (req: AuthRequest, res: Response) => {
...
router.post('/forms', requireAuth, asyncHandler(async (req: AuthRequest, res: Response) => {
  const orgId = req.user?.org_id;
  if (!orgId) throw new AppError(401, 'Unauthorized', 'UNAUTHORIZED');
  ...
  const { data, error } = await supabaseAdmin
    ...
    .insert({ org_id: orgId, title: title.trim(), description, icon: icon||'📋', cover_color: cover_color||'#E01E2C', status:'draft', version:1 })
    .select().single();
  if (error) throw new AppError(500, error.message, 'DB_ERROR');
...
router.patch('/forms/:id', requireAuth, asyncHandler(async (req: AuthRequest, res: Response) => {
  const orgId = req.user?.org_id;
  if (!orgId) throw new AppError(401, 'Unauthorized', 'UNAUTHORIZED');
  ...
  const { data, error } = await supabaseAdmin
    ...
    .from('builder_forms').update(updates).eq('id', req.params.id).eq('org_id', orgId).select().single();
  if (error) throw new AppError(500, error.message, 'DB_ERROR');
  return sendSuccess(res, data);
}));
...
router.delete('/forms/:id', requireAuth, asyncHandler(async (req: AuthRequest, res: Response) => {
  const orgId = req.user?.org_id;
  if (!orgId) throw new AppError(401, 'Unauthorized', 'UNAUTHORIZED');

  await supabaseAdmin.from('builder_forms').delete().eq('id', req.params.id).eq('org_id', orgId);
