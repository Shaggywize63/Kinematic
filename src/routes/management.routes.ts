...
router.post('/', requireRole('admin', 'super_admin', 'main_admin', 'sub_admin'), citiesCtrl.create);

// PATCH /api/v1/cities/:id  → update city (admin/supervisor)
router.patch('/:id', requireRole('admin', 'super_admin', 'main_admin', 'sub_admin'), citiesCtrl.update);

// DELETE /api/v1/cities/:id → delete city (admin)
router.delete('/:id', requireRole('admin', 'super_admin', 'main_admin'), citiesCtrl.remove);
