 ...
 *
 * All functions accept `client_id` for multi-tenant scoping (mirrors the
 * pattern in analytics.service.ts) and an optional `DateRange`.
 *
 * Naming convention: every exported function maps 1:1 to a widget_type
 * stored in crm_dashboard_layouts.config.widgets[].widget_type.
 */
import { supabaseAdmin } from '../../lib/supabase';

export interface DateRange { from?: string; to?: string }

// Apply the client filter to any Supabase query builder. Hard isolation:
// when client_id is provided, returns rows where client_id matches exactly.
...
// ─────────────────────────────────────────────────────────────────
// 1. Lead Velocity Rate — month-over-month % growth in qualified leads
// ─────────────────────────────────────────────────────────────────
export async function leadVelocity(org_id: string, client_id: string | null = null, months_back = 6) {
  ...
  return sorted.map(([month, v], i) => {
    ...
    return {
      ...
      mom_growth_pct: mom == null ? null : Math.round(mom * 10) / 10,
    };
  });
}
...
// ─────────────────────────────────────────────────────────────────
export async function timeToFirstTouch(
  org_id: string,
  ...
  if (!leads?.length) return { avg_minutes: 0, median_minutes: 0, sla_breach_pct: 0, total: 0, breaches: 0, distribution: [] };

  const leadIds = leads.map((l: any) => l.id);
  // For each lead, find the earliest activity (any type).
  ...
  // Distribution buckets: <5m, 5-15m, 15-60m, 1-4h, 4-24h, >24h
  const dist = [
...
// ─────────────────────────────────────────────────────────────────
// 3. Stuck leads — counts at 7/14/30 day thresholds + top stuck owners
// ─────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────
// 4. Top lost reasons (leads with status='lost')
// 5. Top won reasons  (leads with won_reason stamped)
// 6. Disqualification reasons (leads with status='unqualified')
// All three share the same shape — one helper, three call sites.
// ─────────────────────────────────────────────────────────────────
...
export async function wonReasons(org_id: string, client_id: string | null = null, range?: DateRange) {
  ...
  // Graceful fallback when won_reason column hasn't been migrated yet.
  if (error) return [];
  const map = new Map<string, number>();
...
// ─────────────────────────────────────────────────────────────────
// 7. Stage conversion — % advanced between adjacent stages of a pipeline
//    Reads deal-history transitions so a deal that skipped a stage still
//    counts it as "passed through" (mirrors HubSpot stage-conversion).
// ─────────────────────────────────────────────────────────────────
export async function stageConversion(
  ...
  // 2. Pull all deals (with their CURRENT stage + history) in scope.
  ...
  // For each deal, compute the max stage position it ever reached.
  ...
  // 3. For each consecutive stage pair, count deals whose max reached the
  //    earlier stage and the later stage.
...
// ─────────────────────────────────────────────────────────────────
// 8. Lead aging — count of OPEN leads in age buckets
// ─────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────
// 9. Cohort conversion — month cohort × month age conversion matrix
// ─────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────
// 14. Touchpoints-to-response — count of OUTBOUND touches before first
//     inbound response (email reply, call return, whatsapp inbound).
// ─────────────────────────────────────────────────────────────────
export async function touchpointsToResponse(org_id: string, client_id: string | null = null, range?: DateRange) {
  ...
  // Leads with no inbound response yet — bucket as "No response".
...
//     Predictive flag. Pipeline-leakage detector.
// ─────────────────────────────────────────────────────────────────
