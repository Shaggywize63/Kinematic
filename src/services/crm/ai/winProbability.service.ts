...
export async function compute(org_id: string, deal_id: string): Promise<{ probability: number; reasoning: string }> {
  const { data: deal } = await supabaseAdmin.from('crm_deals').select('*').eq('org_id', org_id).eq('id', deal_id).is('deleted_at', null).maybeSingle();
  if (!deal) return { probability: 0, reasoning: 'Deal not found.' };
  const { data: stage } = await supabaseAdmin.from('crm_deal_stages').select('probability, stage_type').eq('id', deal.stage_id).single();
  ...
  const { count: activityCount } = await supabaseAdmin.from('crm_activities')
    .select('id', { count: 'exact', head: true }).eq('deal_id', deal_id).eq('org_id', org_id)
...
async function persist(org_id: string, deal_id: string, p: number, reasoning: string) {
  await supabaseAdmin.from('crm_deals').update({
    win_probability_ai: p, win_probability_reasoning: reasoning, win_probability_updated_at: new Date().toISOString(),
