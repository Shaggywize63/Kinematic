...
export async function summarizeAccount(org_id: string, account_id: string): Promise<string> {
  const { data: account } = await supabaseAdmin.from('crm_accounts').select('*').eq('org_id', org_id).eq('id', account_id).maybeSingle();
  ...
  const summary = await aiComplete({
    ...
    messages: [{ role: 'user', content: JSON.stringify({ account, contacts, deals }) }], max_tokens: 250,
  });
  await supabaseAdmin.from('crm_accounts').update({ ai_summary: summary, ai_summary_updated_at: new Date().toISOString() })
    .eq('id', account_id).eq('org_id', org_id);
  return summary;
}

export async function summarizeDeal(org_id: string, deal_id: string): Promise<string> {
  const { data: deal } = await supabaseAdmin.from('crm_deals').select('*').eq('org_id', org_id).eq('id', deal_id).maybeSingle();
  if (!deal) return 'Deal not found.';
  const { data: activities } = await supabaseAdmin.from('crm_activities').select('type, subject, body, completed_at, status')
  ...
  return aiComplete({
    ...
    messages: [{ role: 'user', content: JSON.stringify({ deal, activities }) }], max_tokens: 250,
  });
}
