...
export async function compute(org_id: string, deal_id: string, force = false): Promise<NextBestAction | null> {
  ...
  if (!force && cacheAgeMs < 6 * 60 * 60 * 1000 && deal.next_action_ai) return deal.next_action_ai as NextBestAction;
  const { data: recent } = await supabaseAdmin.from('crm_activities').select('type, subject, completed_at, status')
    .eq('org_id', org_id).eq('deal_id', deal_id).is('deleted_at', null)
    .order('completed_at', { ascending: false }).limit(5);
  const { data: stage } = await supabaseAdmin.from('crm_deal_stages').select('name, probability, stage_type').eq('id', deal.stage_id).single();
  const userPayload = {
    deal: { name: deal.name, amount: deal.amount, currency: deal.currency, expected_close_date: deal.expected_close_date, created_at: deal.created_at, win_probability_ai: deal.win_probability_ai },
    stage, recent_activities: recent ?? [],
  };
  let nba: NextBestAction;
  try {
    const response = await aiComplete({
      org_id, model: process.env.CRM_NBA_MODEL || 'claude-haiku-4-5',
    ...
    nba = {
      ...
      suggested_template_id: parsed.suggested_template_id ?? null, suggested_when: parsed.suggested_when ?? 'this_week',
    };
  } catch { nba = fallback(deal, stage, recent ?? []); }
  await supabaseAdmin.from('crm_deals').update({ next_action_ai: nba, next_action_updated_at: new Date().toISOString() })
...
function fallback(deal: Record<string, unknown>, stage: Record<string, unknown> | null, recent: unknown[]): NextBestAction {
  ...
  if (stageType === 'won' || stageType === 'lost') return { action: 'nurture', priority: 'low', reason: 'Deal already closed.', suggested_template_id: null, suggested_when: 'next_week' };
  if (recent.length === 0) return { action: 'call', priority: 'high', reason: 'No recent activity. Reach out now.', suggested_template_id: null, suggested_when: 'today' };
