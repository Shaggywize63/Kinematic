...
const CANONICAL_FIELDS = ['first_name','last_name','email','phone','company','title','source','country','city','industry','notes'];

export async function uploadFile(org_id: string, user_id: string | undefined, fileName: string, buffer: Buffer) {
...
async function parseHeadersAndSample(fileName: string, buffer: Buffer) {
  ...
  if (lower.endsWith('.xlsx') || lower.endsWith('.xls')) {
    ...
    ws.getRow(1).eachCell((cell, idx) => { headers[idx - 1] = String(cell.value ?? '').trim(); });
...
async function suggestMapping(org_id: string, headers: string[]) {
  ...
  try {
    const reply = await aiComplete({
      org_id, model: process.env.CRM_LEAD_SCORING_MODEL || 'claude-haiku-4-5',
...
export async function previewJob(org_id: string, job_id: string, mapping: Record<string, string>) {
  const { data: job } = await supabaseAdmin.from('crm_import_jobs').select('*').eq('org_id', org_id).eq('id', job_id).single();
  ...
  if (emails.length) {
    const { data: dups } = await supabaseAdmin.from('crm_leads').select('email').eq('org_id', org_id).in('email', emails).is('deleted_at', null);
    const dupSet = new Set((dups ?? []).map(d => d.email?.toLowerCase()));
    mapped.forEach((r, i) => {
      if (r.email && dupSet.has((r.email as string).toLowerCase())) warnings.push({ row: i, reason: 'Duplicate email' });
    });
  }
...
export async function getJob(org_id: string, job_id: string) {
  const { data, error } = await supabaseAdmin.from('crm_import_jobs').select('*').eq('org_id', org_id).eq('id', job_id).single();
  if (error) throw new AppError(404, 'Import job not found', 'NOT_FOUND');
  return data;
}

export async function listJobs(org_id: string) {
  const { data } = await supabaseAdmin.from('crm_import_jobs').select('*').eq('org_id', org_id).order('created_at', { ascending: false }).limit(50);
  return data ?? [];
}

function mapRow(row: Record<string, unknown>, mapping: Record<string, string>) {
  const out: Record<string, unknown> = {};
  for (const [src, dest] of Object.entries(mapping)) { if (!dest) continue; out[dest] = row[src]; }
  return out;
}
function extractJson(s: string): string { const m = s.match(/\{[\s\S]*\}/); return m ? m[0] : '{}'; }
